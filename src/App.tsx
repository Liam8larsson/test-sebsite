...
function buildSummary(config: Config): string {
  ...
  lines.push(
    `viewer filter: ${config.viewerFilterMode}${config.viewerFilterMode === "tag" ? ` (${config.viewerTag})` : ""}`,
  );
  lines.push(
    `action trigger: ${config.actionTriggerMode}${config.actionTriggerMode === "scoreboard" ? ` / objective=${config.actionScoreboardObjective}` : ""}`,
  );
  ...
  return JSON.stringify(
    {
      ...
      modules: [
        { type: "data", uuid: uid(), version },
        { type: "script", language: "javascript", entry: "scripts/main.js", uuid: uid(), version },
      ],
  ...
  return JSON.stringify(
    {
      ...
      modules: [{ type: "resources", uuid: uid(), version }],
    },
    null,
    2,
  );
...
function buildMainJs(config: Config): string {
  ...
  const tagLines = mapped.map(
    (a, idx) => `${idx === 0 ? "if" : "else if"} (target.hasTag("${a.tag}")) { nextAction = ${a.enumValue}; }`,
  );
  const scoreLines = mapped.map(
    (a, idx) => `${idx === 0 ? "if" : "else if"} (scoreValue === ${a.scoreValue}) { nextAction = ${a.enumValue}; }`,
  );

  const viewerFilter =
    config.viewerFilterMode === "tag"
      ? `if (!player.hasTag("${config.viewerTag}")) {\n      player.setPropertyOverrideForEntity(billboard, "${ns}:show", false);\n      continue;\n    }`
      : "";

  const writeGuard = config.optimizeWrites
    ? `try {\n    if (entity.getProperty(prop) === value) return;\n  } catch {}`
    : "";

  return [
    'import { system, world } from "@minecraft/server";',
    "",
    `const BILLBOARD_ID = "${fullId}";`,
    `const TARGET_TAG = "${config.targetTag}";`,
    `const DEBUG_TAG = "${config.debugTag}";`,
    `const UPDATE_INTERVAL = ${Math.max(1, config.updateInterval)};`,
    `const MAX_TRACK_DISTANCE = ${Math.max(1, config.maxTrackDistance)};`,
    "",
    "function getTargets() {",
    config.targetEntityKind === "player"
      ? '  return [...world.getPlayers()].filter((p) => p.hasTag(TARGET_TAG));'
      : '  return world.getDimension("overworld").getEntities({ tags: [TARGET_TAG] });',
    "}",
    "",
    "function findBillboardFor(target, dimension) {",
    "  const targetId = target.id;",
    "  for (const entity of dimension.getEntities({ type: BILLBOARD_ID, maxDistance: 4, location: target.location })) {",
    '    try { if (entity.getDynamicProperty("targetId") === targetId) return entity; } catch {}',
    "  }",
    "  return null;",
    "}",
    "",
    "function ensureBillboard(target, dimension) {",
    "  let billboard = findBillboardFor(target, dimension);",
    "  if (!billboard) {",
    "    billboard = dimension.spawnEntity(BILLBOARD_ID, target.location);",
    '    billboard.setDynamicProperty("targetId", target.id);',
    "  }",
    "  return billboard;",
    "}",
    "",
    "function setIfChanged(entity, prop, value) {",
    ...
    ...(config.actionTriggerMode === "scoreboard"
      ? [
          "  let scoreValue = 0;",
          `  try { const objective = world.scoreboard.getObjective("${config.actionScoreboardObjective}"); if (objective && target.scoreboardIdentity) scoreValue = objective.getScore(target.scoreboardIdentity) ?? 0; } catch {}`,
          ...scoreLines.map((line) => `  ${line}`),
        ]
      : tagLines.map((line) => `  ${line}`)),
    `  setIfChanged(billboard, "${ns}:a", nextAction);`,
    `  setIfChanged(billboard, "${ns}:dbg", target.hasTag(DEBUG_TAG));`,
    "}",
    "",
    "function applyViewerFilter(billboard) {",
    "  for (const player of world.getPlayers()) {",
    ...(viewerFilter ? [viewerFilter] : []),
    `    player.setPropertyOverrideForEntity(billboard, "${ns}:show", true);`,
    "  }",
    "}",
    "",
    "system.runInterval(() => {",
    ...
    "    const billboard = ensureBillboard(target, dimension);",
    "    syncBillboard(target, billboard);",
    "    applyViewerFilter(billboard);",
    "  }",
    "  for (const billboard of dimension.getEntities({ type: BILLBOARD_ID })) {",
    '    const targetId = billboard.getDynamicProperty("targetId");',
    "    const targetStillExists = targets.some((t) => t.id === targetId && Math.abs(t.location.x - billboard.location.x) <= MAX_TRACK_DISTANCE && Math.abs(t.location.z - billboard.location.z) <= MAX_TRACK_DISTANCE);",
...
function buildClientEntity(config: Config): string {
  ...
  config.actions.forEach((action) => {
    action.directions.forEach((_, i) => {
      textures[textureKey(action.name, i)] = `textures/entity/${textureKey(action.name, i)}`;
    });
  });
  ...
  return JSON.stringify(
    {
      ...
      "minecraft:client_entity": {
        description: {
          identifier: fullId,
          materials: { default: `${ns}.sprite` },
          textures,
          geometry: { default: `geometry.${ns}.${safeName(config.entityId)}` },
          animations: {
...
function buildGeometry(config: Config): string {
  const northUv = { uv: [0, 0], uv_size: [config.textureWidth, config.textureHeight] };
  const southUv = { uv: [config.textureWidth, 0], uv_size: [-config.textureWidth, config.textureHeight] };
  const spriteCube = {
  ...
  return JSON.stringify(
    {
      ...
      [`geometry.${config.namespace}.${safeName(config.entityId)}`]: {
...
function buildRenderController(config: Config): string {
  ...
  mapped.forEach((action) => {
    arrays[action.name] = action.directions.map((_, i) => `Texture.${textureKey(action.name, i)}`);
  });

  const textureExpression =
    mapped.reduce((acc, action) => acc + `q.property('${ns}:a') == ${action.enumValue} ? Array.${action.name}[v.dir_index] : `, "") +
    `Array.${mapped[0]?.name ?? "idle"}[v.dir_index]${")".repeat(Math.max(0, mapped.length - 1))}`;
...
export default function DSMaker() {
  ...
  const autoFillCurrentAction = () => {
    if (!selectedAction) return;
    const cellW = selectedAction.directions[selectedDirIndex]?.textureWidth || 64;
    setConfig((prev) => ({
      ...prev,
      actions: prev.actions.map((action) =>
        action.id !== selectedAction.id
          ? action
          : { ...action, directions: action.directions.map((dir, i) => ({ ...dir, textureX: i * cellW, textureY: 0 })) },
      ),
    }));
  };
  ...
  const autoFillAllActions = () => {
    const cellW = selectedAction?.directions?.[selectedDirIndex]?.textureWidth || 64;
    const cellH = selectedAction?.directions?.[selectedDirIndex]?.textureHeight || 64;
    setConfig((prev) => ({
      ...prev,
      actions: prev.actions.map((action, actionRow) => ({
        ...action,
        directions: action.directions.map((dir, i) => ({ ...dir, textureX: i * cellW, textureY: actionRow * cellH })),
      })),
    }));
  };
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(to bottom, #f8fafc, #eef2f7)", padding: 24, fontFamily: "Arial, sans-serif", color: "#111827" }}>
      <div style={{ maxWidth: 1400, margin: "0 auto", display: "grid", gap: 24 }}>
        <div style={panelStyle}>
          ...
          <p style={{ margin: 0, color: "#64748b" }}>shadcn/ui 없이 바로 돌릴 수 있게 정리한 버전입니다.</p>
        </div>
          ...
          <div style={{ display: "grid", gap: 24 }}>
            <div style={panelStyle}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 14 }}>
                ...
                <div><label style={labelStyle}>전체 배율</label><SimpleInput type="number" step="0.1" value={config.scale} onChange={(e) => updateConfig({ scale: Number(e.target.value) || 1 })} /></div>
            ...
            <div style={panelStyle}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 14 }}>
                <div><label style={labelStyle}>업데이트 주기(틱)</label><SimpleInput type="number" min={1} value={config.updateInterval} onChange={(e) => updateConfig({ updateInterval: Number(e.target.value) || 1 })} /></div>
                <div><label style={labelStyle}>최대 추적 거리</label><SimpleInput type="number" min={1} value={config.maxTrackDistance} onChange={(e) => updateConfig({ maxTrackDistance: Number(e.target.value) || 64 })} /></div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 14 }}>
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 8 }}>
                    <SimpleInput type="number" step="0.01" value={config.spriteOriginX} onChange={(e) => updateConfig({ spriteOriginX: Number(e.target.value) || 0 })} />
                    <SimpleInput type="number" step="0.01" value={config.spriteOriginY} onChange={(e) => updateConfig({ spriteOriginY: Number(e.target.value) || 0 })} />
                    <SimpleInput type="number" step="0.01" value={config.spriteOriginZ} onChange={(e) => updateConfig({ spriteOriginZ: Number(e.target.value) || 0 })} />
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 8 }}>
                    <SimpleInput type="number" step="0.1" value={config.visibleBoundsWidth} onChange={(e) => updateConfig({ visibleBoundsWidth: Number(e.target.value) || 2 })} />
                    <SimpleInput type="number" step="0.1" value={config.visibleBoundsHeight} onChange={(e) => updateConfig({ visibleBoundsHeight: Number(e.target.value) || 3 })} />
                    <SimpleInput type="number" step="0.1" value={config.visibleBoundsOffsetY} onChange={(e) => updateConfig({ visibleBoundsOffsetY: Number(e.target.value) || 1.6 })} />
                  </div>
                </div>
                <div>
                  <label style={labelStyle}>텍스처 시트 W / H</label>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 8 }}>
                    <SimpleInput type="number" value={config.textureWidth} onChange={(e) => updateConfig({ textureWidth: Number(e.target.value) || 512 })} />
                    <SimpleInput type="number" value={config.textureHeight} onChange={(e) => updateConfig({ textureHeight: Number(e.target.value) || 192 })} />
                  </div>
                </div>
                <div>
                  <label style={labelStyle}>디버그 화살표 Z 오프셋</label>
                  <SimpleInput type="number" step="0.01" value={config.debugArrowOffsetZ} onChange={(e) => updateConfig({ debugArrowOffsetZ: Number(e.target.value) || -0.6 })} />
                </div>
              </div>
            </div>
                ...
                <>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 120px auto", gap: 14, marginBottom: 16 }}>
                    ...
                    <div>
                      ...
                      {config.actionTriggerMode === "scoreboard" ? (
                        <SimpleInput type="number" value={selectedAction.scoreValue} onChange={(e) => updateAction(selectedAction.id, { scoreValue: Number(e.target.value) || 0 })} />
                      ) : (
                        <SimpleInput value={selectedAction.tag} onChange={(e) => updateAction(selectedAction.id, { tag: e.target.value })} />
                      )}
                    </div>
                    <div><label style={labelStyle}>우선순위</label><SimpleInput type="number" value={selectedAction.priority} onChange={(e) => updateAction(selectedAction.id, { priority: Number(e.target.value) || 0 })} /></div>
          ...
          <div style={{ display: "grid", gap: 24 }}>
            ...
            <div style={panelStyle}>
              ...
              <div style={{ display: "grid", gap: 10, color: "#64748b", fontSize: 14 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}><ChevronRight size={16} /> 스프라이트 시트를 업로드하고 클릭으로 UV를 맞춘 뒤 결과 텍스트를 복사하면 됩니다.</div>
              </div>
            </div>
          </div>
        </div>
