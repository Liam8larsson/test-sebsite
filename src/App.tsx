...
async function downloadZip(filename: string, files: Record<string, string>) {
  const zip = new JSZip();
...
export default function DSMaker() {
  ...
  const exportFiles = useMemo(() => makeExportFiles(config), [config]);
  ...
  const handleExportZip = async () => {
    try {
      ...
      await downloadZip(`${safeName(config.projectName)}_export.zip`, exportFiles);
    } finally {
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", display: "grid", gap: 24 }}>
        <div style={panelStyle}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 16 }}>
            <SimpleButton primary onClick={handleExportZip} disabled={exportingZip}><Package size={14} /> {exportingZip ? "ZIP 생성 중..." : "ZIP 내보내기"}</SimpleButton>
