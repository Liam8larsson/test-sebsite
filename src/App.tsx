...
function buildGeometry(config: Config): string {
  const result: Record<string, unknown> = { format_version: "1.12.0" };
  ...
  config.actions.forEach((action) => {
    action.directions.forEach((dir, i) => {
      ...
      result[`geometry.${config.namespace}.${entityName}.${textureKey(action.name, i)}`] = {
...
function buildRenderController(config: Config): string {
  ...
  const buildDirExpression = (actionName: string) => {
    const parts = Array.from({ length: config.directionCount }, (_, i) => `v.dir_index == ${i} ? Geometry.${textureKey(actionName, i)} : `).join("");
    return `${parts}Geometry.${textureKey(actionName, 0)}${")".repeat(Math.max(0, config.directionCount - 1))}`;
  };

  const geometryExpression =
    mapped.reduce((acc, action) => acc + `q.property('${ns}:a') == ${action.enumValue} ? ${buildDirExpression(action.name)} : `, "") +
    `${buildDirExpression(defaultAction.name)}${")".repeat(Math.max(0, mapped.length - 1))}`;
                ...
                <div style={{ ...panelStyle, padding: 16 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 12 }}>
                    ...
                    <div>
                      ...
                      <SimpleInput type="number" value={autoGapY} onChange={(e) => setAutoGapY(Number(e.target.value) || 0)} />
                    </div>
                  </div>
                ...
                <div style={{ ...panelStyle, padding: 16 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 12 }}>
                    ...
                    <div>
                      ...
                      <SimpleInput type="number" value={bulkTexY} onChange={(e) => setBulkTexY(Number(e.target.value) || 0)} />
                    </div>
                    <div>
                      <label style={labelStyle}>tex W</label>
                      <SimpleInput type="number" value={bulkTexW} onChange={(e) => setBulkTexW(Number(e.target.value) || 0)} />
                    </div>
                    <div>
                      <label style={labelStyle}>tex H</label>
                      <SimpleInput type="number" value={bulkTexH} onChange={(e) => setBulkTexH(Number(e.target.value) || 0)} />
                    </div>
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 12 }}>
